---
name: hypersku-purchase
display_name: 采购订单管理
display_name_en: Purchase Order Management
description_zh: 通过 hypersku-cli purchase 子命令管理采购订单：按单号查询订单详情、按日期/交易号/物流单号分页搜索订单、查看采购日志与国际物流轨迹。
description_en: Manage purchase orders via hypersku-cli purchase commands: query order details by ID, page through orders by date/trade ID/tracking number, and view purchase logs and international logistics tracking.
description: HyperSKU 采购订单管理。当用户提到订单详情/订单信息/商品明细/搜索订单/订单列表/采购日志/状态流转/国际物流/包裹轨迹时，通过 hypersku-cli purchase 子命令查询对应数据。
version: 2.0.0
author: owen
tags:
  - hypersku
