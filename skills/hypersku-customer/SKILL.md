---
name: hypersku-customer
display_name: 客户管理
display_name_en: Customer Management
...
| 能力 | 用途 | CLI 命令 | 参考文件 |
|------|------|----------|----------|
| 查询客户档案 | 根据客户 ID 查询客户脱敏档案（消费状态、问卷信息、基础档案、渠道来源、业务标签等） | `hypersku-cli customer detail <customerId>` | [customer-detail.md](references/customer-detail.md) |
...
- 用户提到"客户档案/是否有已支付订单/绑定店铺/联系方式/姓名/问卷/DS 经验/广告预算/细分市场/意向服务/客户标签/渠道来源"时，执行 `detail <customerId>` 展示客户脱敏档案。
...
4. **采购状态码**：`info` 输出中的采购状态使用与采购订单相同的状态码对照表（完整对照见采购订单管理能力域 `purchase` 的参考文档 references/info.md）。
