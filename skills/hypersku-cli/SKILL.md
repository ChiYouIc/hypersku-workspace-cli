...
| 能力域 | 用途 | 命令入口 | 承载技能包 |
|--------|------|----------|------------|
| 登录认证 | 登录（token/账密）、登录状态远程校验、退出登录（清除本地 token） | `hypersku-cli auth login/status/logout` | `hypersku-auth` |
...
1. **登录前置**：所有业务查询都要求 token 有效（凭证保存于 `~/.hypersku-cli/config.json` 的 `api_token`）。查询报凭证错误时，先用 `auth status` 远程校验；未登录或已失效则引导用户执行 `auth login`（参见 `hypersku-auth` 技能包）。
