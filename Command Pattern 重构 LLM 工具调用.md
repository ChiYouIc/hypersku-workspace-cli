...
关键在于 MCP 的协议行为：**客户端在连接建立（及工具列表变更）后，会把服务端全部工具的 schema（name + description + 参数定义）收入工具清单，随此后的每一轮请求全量携带**（部分客户端实现为注入 system prompt，属于实现细节）。
...
短期内做不了，做了也不够。先澄清：tool router 本身不是伪命题，在可控客户端或自建网关的场景下，确实可以用前置检索/过滤器按对话裁剪注入的工具集（MCP 生态的 tool search 也是同一方向）。我们没有走这条路，是三层原因：

- **做不了**：router 的裁剪必须作用在注入之前：要么客户端支持按会话动态增删工具，要么自建网关改写 `tools/list`。当时的宿主是百炼、MCP 桌面端这类不可控客户端：schema 在连接建立时全量注入，协议没有留给服务端按会话裁剪的钩子，router 无处落脚；
- **不够**：就算把裁剪链路打通，router 也是同一范式内的优化，不是范式更替："50 个 schema 常驻"优化成"K 个 schema 常驻"，占用仍随可见工具数线性增长；路由错误是硬失败，被裁掉的工具模型根本看不见，兜底只能回退全量注入，问题原样回归；每加一个工具还要同步维护 router 的索引/配置，扩展摩擦只是转移；而且它不触碰授权与审计：授权与执行依旧焊死在 `@McpTool` 里，可见性也至多是被 router 优化了注入集，并未与它解耦；
- **多一跳且有损**：即便给 router 配上 skill（先读文档、再经 router 取工具），链路也是串联的两跳。模型本身具备阅读能力，skill 命中后直接阅读文档即可调用；router 方案则要再经一次关键词/向量检索筛出工具子集、注入 schema、再读一遍：多一跳延迟与出错机会，且检索是有损匹配，筛漏即盲区（展开见 4.5）。
...
先交代真实的演进路线，它没有“先想好模式再找应用”，每一步都是被上一步的痛点推着走的：
...
**第二步得多说两句：Go CLI 的想法来得有点意外，后来又找到了业界佐证。** 被 description 之痛折磨久了，某天突然想到：我们日常给 Agent 用的第三方平台，钉钉（`dws`）、TAPD（`tapd-ai-cli`），不都是“一个命令行入口 + 一份按需加载的技能文档”的形态吗？它们没有向模型暴露几十个工具，却把能力组织得井井有条。查证资料后确认：**“命令 + 技能包”是被多个第三方平台验证过的做法**。于是用 Go + Cobra 开发 `hypersku-cli`，覆盖同样的 14 个业务域。它的定位就是个**试验场**，别当生产工具用：在本地快速试命令的形态、稳定性与可用性，skill 也先对着 CLI 编写和测试，试错成本低、迭代快。命令行有一个被严重低估的特性：**它的帮助是内省的、分层的、按需的**。你不会在登录时被灌输全部 50 个子命令的完整帮助；你 `hypersku-cli --help` 看一眼能力版图，进入某个域再看第二层，具体命令 `--help` 看参数。**人类从来不需要把 CLI 手册背进脑子，为什么模型需要？**

接下来是验证：把 CLI 的命令知识（这个域有哪些命令、参数怎么填）整理成技能包交给宿主（Copilot / Dify 类平台），在一批典型任务上实测，模型拿得又快又准，**命令知识与 skill 的描述式路由刚好是一对**：skill 负责在正确的时机把命令知识注入上下文，命令的 `--help` 负责运行时内省，两层各司其职。可行性验证通过，才有了第三步的封装。
...
同一套命令语法现在有两个宿主：**Go CLI**（试验场：验证命令的稳定性与可用性、编写测试 skill，已覆盖全部 15 个域）与 **Java 进程内命令层**（生产，服务 MCP，当前已落地 6 个域（售后 / 客户 / 国内异常 / 物流 / 采购 / 仓库），其余域渐进迁移中）。命令先在 CLI 上打磨，再固化到 Java 侧，一套心智，两处复用；两侧共用同一套语法规范与域文档事实源，已落地域语义对齐。
...
- **为什么是 1 个而不是 N 个**：一个 string 参数吃下所有命令。schema 是恒定的，不随能力增长而变化，工具数量从 50+ 收敛到 1（少数强 schema 工具仍保留原生 MCP 形态，混合策略见 8.1），命令化能力部分的上下文占用从 O(N) 降到 **O(1)**；
...
skill 只回答一件事：**模型在这个场景下该知道哪些命令的存在。**
...
触发机制是描述式路由：每个技能的 frontmatter `description` 声明"当用户提到 X 时使用本技能"。宿主（Copilot / Dify 类平台）按语义命中后，**只有被命中技能的正文进入上下文**，总览与域技能一视同仁；真正常驻的是全部 16 条技能的 frontmatter `description`，那是宿主做语义命中所依赖的最小集。这就是能力加载的场景驱动。
...
L1  skill（总览 + 域技能包）      常驻：全部技能的 frontmatter description（16 条，随技能数线性增长但量级很小）；正文命中时注入（总览路由表 ~0.5k，域技能 ~0.5-1k/域）
...
还有一层好处藏在文档的写法里。模型看到命令描述的主通道是 skill 文档（SKILL.md 及其 references）：域技能命中注入时，命令的作用、参数含义、输出格式、注意事项（如 trackingNumber 是运单号而非订单号）随文档一起到达模型，看完即调；`CommandDefinition` 的 `description` 是第二通道，`--help` 时按次取用。使用案例则下沉为独立的可取用内容（独立章节或 references 文档），模型自己决定要不要看：简单命令（如查物流轨迹）看完描述即调，不多花一个 token；复杂命令或首次使用时才拉取案例对照。两层通道都不占常驻 token，花销与需求深度成正比，而不是与工具总量成正比。这也是 router 方案给不了的：检索器只决定"注入哪几个工具"，注入的就是全量 schema，没有"先看描述、再看案例"的层次可言。
...
- **执行是进程内方法调用，不走 shell**：`run` 就是一个普通 `Function`，没有 `Runtime.exec`，没有字符串拼接进 shell，没有进程 fork。命令语法在解析层终结，之后的一切都是类型安全的 Java 方法调用。
...
对比 MCP 多工具方案：**可见性**（schema 注入）、**授权**（谁能调）、**执行**（怎么干）全部焊死在同一个 `@McpTool` 注解里。想按客户端隐藏某个工具？严格说倒不是做不到，只是没有声明式的做法：MCP 协议把工具清单定义在 server 粒度（`tools/list` 不按会话裁剪），Spring AI 的注解模式是"注册即全量暴露"。服务端缺少声明式的、按会话裁剪工具清单的钩子；真要按客户端区分，只剩按客户端拆部署实例这类重手段。而改一个参数说明，则是全量客户端的 prompt 同步变化。三层分离后，每件事都可以独立演进：加命令不动 MCP 层，调路由不动命令，改权限不动前两者。
...
- **不复用 OS 权限**：我们不依赖文件系统权限或 OS 用户隔离做防护，边界定义在应用内部，这反而让它可以做到 OS 做不到的粒度。
...
因为命令执行就是方法调用，所以**方法级的一切控制手段都直接可用**。当前落地的是工具组级授权（`@PreAuthorize` + key→authority 映射）；下面四种粒度是这套架构顺手可得、按需启用的能力：
...
- **正确姿势**：skill 管注意力分配（减少误选、降低 token），authority 管准入（拒绝就是拒绝）。我们的架构里二者本来就是分开的：前者在技能包 frontmatter，后者在 `@PreAuthorize` 与 key 配置。这也正是对抗 prompt injection 的底线：恶意网页或对话历史能诱导模型"看见"甚至尝试调用某条命令，但不会让 authority 多放行一个组：可见性可能被注入操纵，准入不会；
- **权限拒绝如何反馈给模型**：拒绝由 Spring Security 的方法级校验（`@PreAuthorize` 在工具方法调用时触发，而非过滤器层，后者只处理认证失败）产生，经 MCP 工具调用的错误通道返回模型。设计原则是与"参数错误"区分语义：参数错可重试，权限拒绝应让模型停止重试、转告用户。结构化的语义化拒绝文案仍在开发中（与 8.4 的权限改造同批推进），当前版本返回框架默认消息。
...
| 上下文占用 | 裁剪后仍 O(可见工具数)个 schema 常驻 | O(1) 常驻 + 按需注入 |
| 路由链路 | skill 命中后仍需 router 二次检索注入（串联两跳） | skill 命中后模型直接阅读域文档即调（一跳） |
| 选择准确性 | 检索式（关键词/向量）有损匹配，筛漏即盲区 | 模型原生阅读语义无损，`--help` 运行时内省兜底 |
...
"路由链路"与"选择准确性"两行展开说说，这是 skill + command 相对 router 方案的结构性优势：模型本身具备阅读能力，skill 被宿主语义命中后，文档内容就是模型要的命令知识，读完即调，整条链路是"一次语义路由 + 模型原生阅读"。而 skill + router 是串联两跳：宿主先按语义命中 skill，模型读完后还要经 router 的检索（关键词/向量/分类模型）二次筛出工具子集、注入 schema、再读一遍。多一跳的延迟，也多一跳出错机会；且第二跳是有损的机器检索：关键词受同义/多义干扰，向量有语义漂移，相似度阈值截断后筛漏的工具模型根本看不见。让模型直接阅读，用的是它最强的语义理解；让检索器替模型选工具，等于把链路里最强的环节外包给了最弱的一跳。

审计要单独说一句：`run_cli` 的入口收敛让全系统的执行流只有一个咽喉要道，`logger.info("run command: {}", cliCommand)` 一行日志记录的是"执行了什么"（命令+参数）；"谁"（来源 key）与"何时"在同一请求上下文里（与访问日志 / MDC 关联即可对齐），结果状态可由返回值补记。单入口的真正含义是审计要素自动聚到了一处，多工具方案的同样要素要横跨 N 个工具分散埋点。
...
自然语言 → 结构化 args 的映射靠两层完成：模型产出命令字符串（利用其命令行语料先验），`CommandDefinition` 负责解析。类型安全与校验位置的选择：**解析层做结构性校验**（必填项、option 名合法性、位置参数个数），**方法层做业务校验**（订单号格式、数据归属）。校验失败的反馈面向模型设计：不抛异常栈，返回 `help()` 文本或一句"缺少必填参数 trackingNumber"，模型读得懂，就能自我修正。另一个隐性收益是解析的宽容度：面向模型的文档只教 `--opt=value` 一种范式，但解析层实际同时兼容 `--opt value` 与 `--flag`，模型偶尔写串也不会失败。范式收敛是给模型减负，不是给系统加脆性。

### 5.3 skill 的粒度与触发

一个 skill 对应**一个能力域**（不是一条命令，也不贪多求全一个 skill 装全部）。触发靠 frontmatter `description` 的描述式路由（"当用户提到物流/轨迹/快递/包裹时"），由宿主做语义命中——规则触发容易漏，全靠模型判断又贵，描述式路由是二者之间最好的平衡。多 skill 并存时上下文预算：常驻的是全部技能的 frontmatter `description`（16 条，随技能数线性增长，这里主动说破，但量级远小于 N 个工具 schema）；技能正文按需加载（总览路由表约 500 token，域技能每个 0.5~1k token），同时激活的域通常不超过 2~3 个。
...
进程内方法调用的另一份收益：**命令级单测直接就能写**。`CommandDefinition` 是纯对象，注册一棵小树、喂参数、断言输出，不需要起 MCP 服务、不需要 mock HTTP 层；Go 侧核心域配套单测（15 个 API 文件中 10 个已有 `_test.go`，其余域测试补齐中），同一套命令语义在两个宿主里各自可测。集成验证则简单到极致：`run_cli` 一个入口，curl 一发命令字符串即可回归。
...
真实多步任务："客户反馈订单 3 天没动，帮我查一下订单状态和最新物流，如果是异常件顺便看下有没有售后工单。"它需要客户订单、物流轨迹、售后三个能力域，典型的跨域任务。
...
- 模型直接产出：
  ...
  ```
- 2 次调用全部命中，无纠错轮次；
- 说明：本次执行中任务的条件分支（"如果是异常件顺便看下售后工单"）未触发：订单非异常件，售后域技能未注入，第三步未执行。
...
| 有效调用率 | 3/5 | 2/2 |
| 首 token 延迟 | 更高（长 prompt 拖慢推理） | 更低 |
| 授权粒度 | 工具组级 `@PreAuthorize`（粒度固定于工具） | 同样落地工具组级，但可扩展到命令 / 参数 / 子树级（见 4.3） |
| 扩展成本 | 加 schema + 全客户端 prompt 变化 | 加一个 `CommandProvider` 类 |
| 开发维护 | 说明是注解里的自由字符串，改说明即改全量客户端 prompt，说明长一分常驻 token 涨一分 | 四件收敛到应用内：说明是 CommandDefinition 的类型化声明字段（定义层结构化），维护只改应用内的类与技能文档（不碰 MCP 协议层），说明详写不占常驻 token；运行时类型安全让位给 schema 工具（见 8.1） |

再补一笔 token 之外的账，这套方案从工程四维看也优于直接用 `@McpTool`：

- **工具说明**：`@McpTool` 的 description 只是注解里的一个字符串，长度、结构全无约束，容易写成一句话笼统描述或无节制长文；`CommandDefinition` 的 `shortDesc` + `description` + `options`/`arguments` 的逐项说明是结构化字段，写清楚作用、参数含义、输出格式是它的本职；
- **代码规范**：优势收敛在定义层，`CommandDefinition` 把名称、说明、选项、位置参数收进类型化的声明字段，命令以类为单位组织；工具侧的说明则只是注解里的自由字符串。同时要说清反向的一面：运行时类型安全在工具侧：`@McpTool` 的参数是类型化的 Java 方法参数，命令侧 `run` 是 `Function<Map<String,String>, String>`，运行时靠字符串键取参（见 5.2），这正是 8.1 保留强 schema 原生工具的理由之一；
- **维护**：前者改一行说明就是改全量客户端的 prompt，回归风险面大；后者的命令知识首要通道是 skill 文档，改命令参数说明要同步改 SKILL.md。维护是"类 + 技能文档"两件套，但都收敛在应用内，不触碰 MCP 协议层，发布即生效；
- **token**：前者 description 写多长，常驻 token 就涨多少，详写与省钱此消彼长；后者说明写在 CommandDefinition 里，只在 `--help` 时按次返回，想写多详细都不占常驻 token，详细与省钱不再冲突。
...
| 拒绝体验 | 拒绝消息语义化为"该 key 无权执行此命令组"（设计定稿，实现随 8.4 权限改造推进中），目标行为：模型从重试转为转告用户 |
...
- **需要复杂 schema 的场景**：如果某能力真的需要十几个强类型参数、嵌套结构、严格枚举——结构化 MCP 工具的 schema 约束反而比字符串命令更稳。我们的做法是混合保留：`CliTools` 之外，`LogisticsTools` 等少数强 schema 工具仍然以原生 MCP 工具形态存在，各取所长。这也是临界点的另一种体现：命令与工具是分工关系，谈不上替代。
...
- **命令使用埋点**：基于 `run_cli` 审计日志沉淀"模型实际用哪些命令"，反哺技能包裁剪与命令粒度优化，形成数据闭环。
...
同一套命令语法规范（命令路径 + 位置参数在前 + `--opt=value` 在后），域文档（skills）单一事实源，两侧各自实现、各自配套单测（Go 侧 `internal/apis/*_test.go`，Java 侧注册小命令树断言输出），已覆盖域的语义漂移被测试挡在合入前。Go 版是试验场：先验证命令的稳定性与可用性、配套 skill 的编写与测试；Java 版是生产形态，服务 MCP。覆盖上如实交代：Java 侧当前已落地 6 个域（售后/客户/国内异常/物流/采购/仓库），其余域渐进迁移中，已落地域语义对齐。
