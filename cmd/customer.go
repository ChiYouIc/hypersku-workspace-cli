...
// 查询客户信息
var getCustomerInfoCmd = &cobra.Command{
	Use:   "info [customerId]",
	Short: "查询客户信息",
	Long:  "通过客户 ID 查询客户基础信息，包含等级、最近 30 天订单数量、店铺数量、从事 Dropshipping 时长、周广告预算、月订单量、客户来源等。",
	Args:  cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		customerId := args[0]
		info, err := apis.NewCustomerInfoApi().GetCustomerInfo(customerId)
		if err != nil {
			cmd.PrintErrf("查询客户 %s 信息失败：%v\n", customerId, err)
			return
		}
		if info == nil {
			cmd.Print("未查询到客户信息")
